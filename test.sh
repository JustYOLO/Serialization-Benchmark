...
BASE_CONFIG="8, string, 8, 8, 16, 16, 100, FB"
...
SERIALIZATION_FUNCS=("JSON" "MP" "FX" "PB" "TH" "FB")
...
run_test() {
    ...
    echo "# nkey, tkey, skeyMin, skeyMax, svalMin, svalMax, testSize, serialization_function
...
# Change serialization functions
for func in "${SERIALIZATION_FUNCS[@]}"; do
    echo "running: $func"
    # Change key types
    for type in "${KEY_TYPES[@]}"; do
        config="8, $type, 8, 8, 16, 16, 100, $func"
        run_test "$config"
    done

    # Change number of keys
    for keys in "${NUM_KEYS[@]}"; do
        config="$keys, string, 8, 8, 16, 16, 100, $func"
        run_test "$config"
    done

    # Change key sizes
    for key_size in "${KEY_SIZES[@]}"; do
        config="8, string, $key_size, $key_size, 16, 16, 100, $func"
        run_test "$config"
    done

    # Change value sizes
    for value_size in "${VALUE_SIZES[@]}"; do
        config="8, string, 8, 8, $value_size, $value_size, 100, $func"
        run_test "$config"
    done
done
